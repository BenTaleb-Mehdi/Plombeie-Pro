    ...
    <>
      <section className="relative bg-slate-950 text-white pt-28 pb-0 overflow-hidden">
        ...
        <div className="relative z-20 mx-auto max-w-6xl px-4 sm:px-6 pb-12 pt-6">
          <Link
            href="/portfolio"
            className="inline-flex items-center gap-2 text-sm font-semibold text-slate-400 hover:text-white transition-colors mb-6 sm:mb-8"
          >
            <ArrowLeft className="h-4 w-4" />
            {t("portfolio_page.back_to")}
          </Link>

          <div className="inline-flex items-center gap-1.5 rounded-full bg-sky-500/15 border border-sky-500/25 px-3 py-1 text-xs font-semibold tracking-widest text-sky-400 uppercase mb-3 sm:mb-4">
          ...
          <div className="mt-4 sm:mt-6 flex flex-wrap gap-3 sm:gap-4">
            {[
              ...
              <div key={value} className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-slate-300 font-medium">
                <Icon className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-sky-400 shrink-0" />
                {value}
              </div>
            ))}
          </div>
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <div className="grid gap-8 lg:grid-cols-[1fr_380px] xl:grid-cols-[1fr_420px]">
            ...
            <div className="flex flex-col gap-6 sm:gap-8">
              <div className="relative w-full overflow-hidden rounded-2xl sm:rounded-3xl bg-slate-100 shadow-xl"
                ...
                {images.length > 1 && (
                  <>
                    <button
                      ...
                      className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full bg-white/90 shadow-md hover:bg-white text-slate-800 transition-all hover:scale-105 active:scale-95 cursor-pointer z-10"
                    ...
                    <button
                      ...
                      className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full bg-white/90 shadow-md hover:bg-white text-slate-800 transition-all hover:scale-105 active:scale-95 cursor-pointer z-10"
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 -mx-4 sm:mx-0 px-4 sm:px-0">
                  {images.map((img, idx) => (
                    <button
                      ...
                      className={`relative shrink-0 h-14 w-20 sm:h-16 sm:w-24 rounded-xl overflow-hidden border-2 transition-all duration-200 cursor-pointer ${
                        idx === activeIdx
              ...
              <div className="grid grid-cols-2 gap-2 lg:hidden">
                {[
                  { label: t("modal.location"), value: translatedLocation, Icon: MapPin },
                  { label: t("modal.duration"), value: translatedDuration, Icon: Clock },
                  { label: t("modal.date"), value: translatedDate, Icon: Calendar },
                  { label: t("portfolio_page.category_label"), value: translatedCategory, Icon: Tag },
                ].map(({ label, value, Icon }) => (
                  <div key={label} className="flex items-center gap-2 rounded-xl bg-slate-50 p-3 border border-slate-100">
              ...
              <div className="flex flex-col gap-2 lg:hidden">
                <a
                  ...
                  className="flex items-center justify-center gap-2 w-full rounded-full bg-slate-950 px-6 py-3.5 text-xs font-black uppercase tracking-wider text-white shadow-lg hover:bg-slate-800 transition-all duration-300 active:scale-95"
                >
                ...
                <Link
                  href="/contact"
                  className="flex items-center justify-center gap-2 w-full rounded-full border border-slate-200 bg-white px-6 py-3 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all duration-200"
                >
              ...
              <div className="bg-slate-50 rounded-2xl sm:rounded-3xl p-5 sm:p-8 border border-slate-100">
                <h2 className="text-base sm:text-lg font-black text-slate-900 mb-2 sm:mb-3">{t("modal.description")}</h2>
              ...
              {item.features && item.features.length > 0 && (
                <div className="flex flex-col gap-3">
                  <h2 className="text-base sm:text-lg font-black text-slate-900">{t("modal.works")}</h2>
                  <div className="grid gap-2 sm:grid-cols-2">
                    {item.features.map((_, idx) => (
                      <motion.div
                        ...
                        className="flex items-start gap-2.5 sm:gap-3 bg-white rounded-xl sm:rounded-2xl p-3 sm:p-3.5 border border-slate-100 shadow-sm"
                      >
              ...
              {portfolio.filter((p) => p.id !== item.id).length > 0 && (
                        ...
                        .map((p) => {
                          ...
                          return (
                            <Link
                              key={p.id}
                              href={`/portfolio/${p.id}`}
                              className="flex items-center gap-3 rounded-xl bg-white p-2.5 border border-slate-100 hover:border-sky-200 hover:shadow-sm transition-all duration-200 group"
                            >
